...
def convert_ebird_to_artsobs(submission_id:str) -> pd.DataFrame:
    df = get_ebird_data()
    ...
    # save to file
    output_path = root_dir / 'artsobs_output'
    output_path.mkdir(exist_ok=True)
    output_filename = output_path / f"{location_name.iloc[0]}_{ebird_date.iloc[0]}.xlsx"
    artsobs_df.to_excel(output_filename, index=False)

if __name__ == "__main__":
    submission_id = 'S357827245'
    convert_ebird_to_artsobs(submission_id)
